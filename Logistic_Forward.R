...
forward = step(min.model1, direction='forward', scope=max.formula.model1)
summary(forward)

# predict Churn in Training dataset
mm<-predict.glm(forward, new=train,type="response",se.fit=TRUE)
train$pred <- mm$fit
...
mtest<-predict.glm(forward, new=test,type="response",se.fit=TRUE)
test$pred <- mtest$fit
...
train_prob <- predict(forward, data = train, type = "response") 
test_prob <- predict(forward, newdata = test, type = "response")
